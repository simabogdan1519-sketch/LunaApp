...
class _LunaNavBarState extends State<_LunaNavBar>
  ...
  double _dragStart = 0;
  static const double _kSwipeThreshold = 40;
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      // Detect horizontal swipe on the nav bar itself
      ...
      onHorizontalDragEnd: (d) {
        ...
        if (delta.abs() > _kSwipeThreshold) {
          // swipe right → go to previous group (dir = -1)
          // swipe left  → go to next group    (dir = +1)
          widget.onSwipe(delta < 0 ? 1 : -1);
        }
      },
      child: Container(
